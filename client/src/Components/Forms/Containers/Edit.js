...
class Edit extends Component {
  ...
  handleEdit = (e) => {
    fetch(`http://localhost:4000/API/reports/report/${this.state.Report.ID}`, {method: 'post', body: JSON.stringify(this.state.Report), headers: {"Content-Type": "application/json"}})
  ...
  render() {
    ...
    <div className = 'container' >
        ...
        <form>
          ...
          <Link to = {{pathname: `/`}} onClick = {this.handleEdit}><button  className = 'btn btn-success' style = {{marginLeft: '5px'}}> Submit </button></Link>
        </form>
        </div>
    </div>
    )
